...
public class ToxicLake extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (!world.getBlockState(origin.below()).isSolid()) return false;
                ...
                if (distance <= effectiveRadius) {
                    ...
                    for (int y = 0; y >= -currentDepth; y--) {
                        ...
                        if (world.getBlockState(pos).isSolid()) {
                            world.setBlock(pos, Blocks.CAVE_AIR.defaultBlockState(), 3);
                            airBlocks.add(pos);
                        }
                    }
                }
        ...
        int waterLevel = origin.getY() - maxDepth + 4;
        for (BlockPos airPos : airBlocks) {
            if (airPos.getY() <= waterLevel) {
                world.setBlock(airPos, EndlessStoreFluids.ACID_BLOCK.get().defaultBlockState(), 3);
            }
        }
