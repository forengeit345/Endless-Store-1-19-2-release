...
public class JackEntity extends HostileEntity implements GeoEntity {
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if (tAnimationState.isMoving() && !this.handSwinging) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.jack.jump", Animation.LoopType.LOOP));
...
//        TODO: починить анимации джэка чтобы он перестал вертеть головой
            ...
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.jack.idle", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        else {
    ...
    protected void damage(LivingEntity target) {
        if (this.isAlive()) {
            if (this.squaredDistanceTo(target) < 0.8 * 0.8 && this.canSee(target) && target.damage(this.getDamageSources().mobAttack(this), this.getDamageAmount())) {
    ...
    static class SwimmingGoal extends Goal {
        ...
        public boolean canStart() {
            return (this.jack.isTouchingWater() || this.jack.isInLava()) && this.jack.getMoveControl() instanceof JackMoveControl;
        }
        ...
        public void tick() {
            ...
            ((JackMoveControl)this.jack.getMoveControl()).move(1.2);
        }
    }
    ...
    static class FaceTowardTargetGoal extends Goal {
        ...
        public void tick() {
            ...
            ((JackMoveControl)this.jack.getMoveControl()).look(this.jack.getYaw(), this.jack.canAttack());
        }
    }
    ...
    static class RandomLookGoal extends Goal {
        ...
        public boolean canStart() {
            return this.jack.getTarget() == null && (this.jack.isOnGround() || this.jack.isTouchingWater() || this.jack.isInLava() || this.jack.hasStatusEffect(StatusEffects.LEVITATION)) && this.jack.getMoveControl() instanceof JackMoveControl;
        }
    ...
    static class MoveGoal extends Goal {
        ...
        public void tick() {
            ((JackMoveControl)this.jack.getMoveControl()).move(1.0);
        }
    }
